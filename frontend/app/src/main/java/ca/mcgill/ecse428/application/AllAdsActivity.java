...
public class AllAdsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sortButton.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isChecked) {
                    fetchAdsByZipcodeAsc();
                } else {
                    sortButton.setTextOff("ZIPCODE DESC");
                    fetchAdsByZipcodeDesc();
                }
            }
        });
    ...
    public void fetchAdsByZipcodeAsc(){
        backend.getAllAdsByZipcodeAsc()
                .subscribeOn(Schedulers.newThread())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<List<AdDto>>() {
                    @Override
    ...
    public void fetchAdsByZipcodeDesc(){
        backend.getAllAdsByZipcodeDesc()
                .subscribeOn(Schedulers.newThread())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<List<AdDto>>() {
                    @Override
    ...
    public void startRecyclerView(List<AdDto> adDtos){
        ...
        AdsRecyclerAdapter adapter = new AdsRecyclerAdapter(adDtos, username,token,AllAdsActivity.this);
        adsRecyclerView.setAdapter(adapter);
        adsRecyclerView.setLayoutManager(new LinearLayoutManager(AllAdsActivity.this));
    }
